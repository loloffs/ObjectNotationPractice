const library = {
  ...
  printPlaylists: function() {
    ...
    for (const key in this.playlists) {
      result += `${key}: ${this["playlists"][key]["name"]} - ${this["playlists"][key]["tracks"].length} track(s)\n`;
    }
    return result;
  },
  ...
  printTracks: function() {
    ...
    for (const key in this.tracks) {
      result += `${key}: ${this["tracks"][key]["name"]} by ${this["tracks"][key]["artist"]} (${this["tracks"][key]["album"]})\n`;
    }
    return result;
  },
  
  printPlaylist: function(playlistId) {
    let result = `${this['playlists'][playlistId].id}: ${this['playlists'][playlistId].name} - ${this['playlists'][playlistId]['tracks'].length} tracks\n`;
    for (const key of this.playlists[playlistId]["tracks"]) {
      for (const x in this.tracks) {
        if (x === key) {
         result += `${x}: ${this["tracks"][x]["name"]} by ${this["tracks"][x]["artist"]} (${this["tracks"][x]["album"]})\n`;
        }
      }
    }
    return result
  ...
  addTrackToPlaylist: function(trackId, playlistId) {
    const allTracks = Object.keys(this["tracks"]);
